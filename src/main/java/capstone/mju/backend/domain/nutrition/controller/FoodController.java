...
public class FoodController {
    ...
    @Operation(
            ...
            responses = {
                    @ApiResponse(responseCode = "200", description = "상품명 검색 성공", content = @Content(mediaType = "application/json", schema = @Schema(implementation = FoodNameResponseDto.class))),
    ...
    @Operation(
            ...
            responses = {
                    @ApiResponse(responseCode = "200", description = "영양성분 검색 성공", content = @Content(mediaType = "application/json", schema = @Schema(implementation = FoodNamedetailResponse.class))),
                    @ApiResponse(responseCode = "400", description = "잘못된 요청")
            }
    )
    ...
    public ResponseEntity<List<FoodNameResponseDto>> searchByItemReportNo(
            @Parameter(description = "검색할 품목제조번호", required = true) @RequestParam String itemReportNo,
            @Parameter(description = "페이지 번호", example = "0") @RequestParam(defaultValue = "0") int page
    ) {
    ...
    @Operation(
            summary = "품목제조보고 번호로 영양성분 검색",
            description = "주어진 품목제조번호로 영양성분을 검색합니다.",
            responses = {
                    @ApiResponse(responseCode = "200", description = "영양성분 검색 성공", content = @Content(mediaType = "application/json", schema = @Schema(implementation = FoodResponseDto.class))),
                    @ApiResponse(responseCode = "400", description = "잘못된 요청")
            }
    )
